...
class Welcome extends Step
{
    ...
    public function __construct($plugin = \false)
    {
        ...
        $this->set_id('welcome');
        $this->set_name(\__('Welcome'));
        $this->set_title(\__('Welcome to Posts Table Pro'));
        $this->set_description(\__('Create and display beautiful tables of your website content'));
    ...
    {
        $fields = [['type' => 'license', 'label' => \__('License key'), 'name' => 'license', 'value' => '']];
        return $fields;
    }
    /**
     * {@inheritdoc}
    ...
    public function save_data($request)
    {
        ...
        if (!$license_details) {
            return $this->send_error_response(['message' => \__('No license details provided. Please enter a license.')]);
        }
        $status = isset($license_details['status']) ? $license_details['status'] : \false;
        if ($status !== 'active') {
            return $this->send_error_response(['message' => \__('Please validate your license.')]);
        }
