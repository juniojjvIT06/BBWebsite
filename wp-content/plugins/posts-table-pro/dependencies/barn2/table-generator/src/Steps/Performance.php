...
class Performance extends Step
{
    ...
    public function __construct($plugin = \false)
    {
        ...
        $this->set_name(\__('Performance'));
        $this->set_title(\__('Performance'));
        $this->set_description(\__('Optimize your table load times.'));
    ...
    {
        $fields = [['type' => 'heading', 'label' => \__('Lazy load'), 'tag' => 'h2'], ['type' => 'checkbox', 'label' => \__('Load table one page at a time'), 'name' => 'lazyload', 'description' => \__('Enable this if the table will contain a large number of %contentType%.')]];
        return $fields;
    }
    /**
     * {@inheritdoc}
    ...
    public function save_data($request)
    {
        ...
        $table_id = $request->get_param('table_id');
        if (empty($table_id)) {
            return $this->send_error_response(['message' => \__('The table_id parameter is missing.')]);
        }
