...
class Shortcode
{
    ...
    {
        $args = $this->args_resolver::get_table_defaults();
        ...
        if (!empty($table->get_post_status())) {
            $args['status'] = $table->get_post_status();
        }
        ...
        // Inject the "exclude terms" parameter.
        if (!empty($table->get_terms(\false, \true))) {
            $args['exclude_term'] = $table->get_terms(\true, \true);
        }
