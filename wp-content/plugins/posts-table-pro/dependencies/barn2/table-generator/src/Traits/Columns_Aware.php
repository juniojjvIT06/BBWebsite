...
trait Columns_Aware
{
    ...
    public function get_columns_list($content_type = '')
    {
        ...
        foreach ($columns as $key => $column) {
            ...
            if ($key === 'button') {
                $formatted[$key] = \__('Button');
            } else {
                if ($key === 'date_modified') {
                    $formatted[$key] = \__('Last modified date');
                }
            }
        }
        ...
        $formatted = Util::insert_into_array($formatted, 'status', 'cf', \__('Custom field'), \false, \true);
        return $formatted;
    }
    /**
    ...
    private function get_taxonomies($post_type)
    {
        ...
        foreach ($registered as $taxonomy) {
            ...
            $taxonomies['tax:' . $taxonomy->name] = $taxonomy->label;
        }
        return $taxonomies;
    }
}
